...
public interface CollectedLogsRepository extends JpaRepository<CollectedLog, UUID> {
    @Query("select l from collected_logs l where service_id in :ids AND path LIKE %:path% AND log like %:searchString%")
    ...
    @Query("select l from collected_logs l where service_id in :ids and timestamp >= :timestampFrom AND path LIKE %:path% AND log like %:searchString%")
    ...
    @Query("select l from collected_logs l where service_id in :ids and timestamp <= :timestampTo AND path LIKE %:path% AND log like %:searchString%")
    ...
    @Query("select l from collected_logs l where service_id in :ids and timestamp >= :timestampFrom and timestamp <= :timestampTo AND path LIKE %:path% AND log like %:searchString%")
