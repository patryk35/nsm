...
public interface MonitoredParametersValuesRepository extends JpaRepository<MonitoredParameterValue, Long> {
    @Query("select l from parameters l where service_id in :ids AND parameter_type_id = :paramId")
    ...
    @Query("select l from parameters l where service_id in :ids AND parameter_type_id = :paramId and timestamp >= :timestampFrom")
    ...
    @Query("select l from parameters l where service_id in :ids AND parameter_type_id = :paramId and timestamp <= :timestampTo")
    ...
    @Query("select l from parameters l where service_id in :ids AND parameter_type_id = :paramId and timestamp >= :timestampFrom and timestamp <= :timestampTo")
